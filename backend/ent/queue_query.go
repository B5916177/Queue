...
func (qq *QueueQuery) QueryEmployee() *EmployeeQuery {
	query := &EmployeeQuery{config: qq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QueueQuery) WithEmployee(opts ...func(*EmployeeQuery)) *QueueQuery {
	query := &EmployeeQuery{config: qq.config}
	for _, opt := range opts {
		opt(query)
	}
	qq.withEmployee = query
	return qq
}
...
func (qq *QueueQuery) sqlAll(ctx context.Context) ([]*Queue, error) {
	...
	if qq.withDentist != nil || qq.withEmployee != nil || qq.withPatient != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := qq.withEmployee; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Queue)
		for i := range nodes {
