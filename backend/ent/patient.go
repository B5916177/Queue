...
func (pa *Patient) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field patient_gender", values[1])
	} else if value.Valid {
		pa.PatientGender = value.String
	}
	if value, ok := values[2].(*sql.NullInt64); !ok {
		return fmt.Errorf("unexpected type %T for field patient_age", values[2])
	} else if value.Valid {
		pa.PatientAge = int(value.Int64)
	}
	if value, ok := values[3].(*sql.NullInt64); !ok {
		return fmt.Errorf("unexpected type %T for field patient_phone", values[3])
	} else if value.Valid {
