...
func (eq *EmployeeQuery) QueryQueue() *QueueQuery {
	query := &QueueQuery{config: eq.config}
...
func (eq *EmployeeQuery) WithQueue(opts ...func(*QueueQuery)) *EmployeeQuery {
	query := &QueueQuery{config: eq.config}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withQueue; query != nil {
		...
		query.Where(predicate.Queue(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.QueueColumn, fks...))
		}))
